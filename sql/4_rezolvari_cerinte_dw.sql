...
-----5
-- ============================================================================
-- CONSTRANGERE DE UNICITATE (GRANULARITATE)
-- ============================================================================

ALTER TABLE FACT_EXHIBITION_ACTIVITY
  ADD CONSTRAINT uk_fact_granularity
  UNIQUE (ARTWORK_KEY, EXHIBITION_KEY, DATE_KEY)
  DISABLE VALIDATE;

-- ============================================================================
-- CHEI EXTERNE (FOREIGN KEYS) - TABELA DE FAPTE
-- ============================================================================
...
-- Verificare masuri pozitive
ALTER TABLE FACT_EXHIBITION_ACTIVITY
  ADD CONSTRAINT ck_fact_insured_pos
  CHECK (INSURED_AMOUNT >= 0)
  ENABLE NOVALIDATE;
  
-----6
-- ============================================================================
-- 1. Index BITMAP pe tipul loca?iei (R?mâne neschimbat, e corect)
-- Cardinalitate redusa (Exhibit, Storage)
-- ============================================================================

CREATE BITMAP INDEX bix_dim_location_type 
ON DIM_LOCATION(TYPE);

-- TESTARE PERFORMANTA (EXPLAIN PLAN)
EXPLAIN PLAN FOR
SELECT 
    l.NAME,
    SUM(f.ESTIMATED_VALUE) as Valoare_Totala
FROM FACT_EXHIBITION_ACTIVITY f
JOIN DIM_LOCATION l ON f.LOCATION_KEY = l.LOCATION_KEY
WHERE l.TYPE = 'Exhibit'
GROUP BY l.NAME;
...
CREATE BITMAP INDEX bix_fact_loan_flag 
ON FACT_EXHIBITION_ACTIVITY(LOAN_FLAG);

-- TESTARE PERFORMANTA (EXPLAIN PLAN)
-- Scenariu: Vrem sa vedem valoarea asigurata doar pentru operele imprumutate

EXPLAIN PLAN FOR
SELECT 
    d.CALENDAR_YEAR,
    COUNT(*) as Numar_Opere_Imprumutate,
    SUM(f.INSURED_AMOUNT) as Total_Asigurat
FROM FACT_EXHIBITION_ACTIVITY f
JOIN DIM_DATE d ON f.DATE_KEY = d.DATE_KEY
WHERE f.LOAN_FLAG = 1 -- Filtrul care foloseste indexul BITMAP
