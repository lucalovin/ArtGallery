...
-- ETL_SYNC table for tracking ETL synchronization operations
-- Run this script in the ART_GALLERY_OLTP schema

CREATE TABLE Etl_Sync (
    id NUMBER GENERATED ALWAYS AS IDENTITY
        CONSTRAINT pk_etl_sync PRIMARY KEY,
    sync_date TIMESTAMP NOT NULL,
    status VARCHAR2(50) DEFAULT 'Pending' NOT NULL,
    records_processed NUMBER DEFAULT 0 NOT NULL,
    records_failed NUMBER DEFAULT 0 NOT NULL,
...
CREATE INDEX idx_etl_sync_sync_date ON Etl_Sync(sync_date);
CREATE INDEX idx_etl_sync_status ON Etl_Sync(status);
